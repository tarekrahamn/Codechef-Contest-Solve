...
void Broken_Heart(int t){
   ...
   cin >> s;
    ...
    hay[0] = 1;
   fora(i,1,n) {
        hay[i] = (hay[i - 1] *lagbo) % MOD;
    }
    dp[0] = 0;
    rof(i,n-1,0){
        dp[i] = (dp[i + 1] + (s[i] - 'a' + 1) * hay[i]) % MOD;
    }
    auto fun = [&](int l, int r) -> int {
        int value = ((dp[r] - dp[l] + MOD) % MOD * hay[n - 1 - l]) % MOD;
        return value;
    };
   ...
   int count = 0;
     fora(i,0,n) {
        ...
        if (fun(0, i) == fun(i, 2 * i) and  fun(2 * i, 2 * i + rem / 2) == fun(2 * i + rem / 2, n)) {
            count++;
        }
    }
