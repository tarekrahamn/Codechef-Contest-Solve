...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------*/
void dfs(int node, const vector<vector<int>>& adj, vector<int>& b, vector<bool>& visited, vector<int>& ans, int ind) {
    visited[node] = true;
    for (int neighbor : adj[node]) {
        if (!visited[neighbor]) {
            dfs(neighbor, adj, b, visited, ans, ind);
            b[node] += b[neighbor];
        }
    }
    if (b[node] == 1 and  node != ind) {
        ans.push_back(node);
        b[node]--;
    }
}
void Broken_Heart(int t) {
    ...
    // dfs(ind, adj, b, visited, ans, ind);
    queue<int> tre;
     int ind =  distance(a.begin(), min_element(all(a)));
    fora(i,0,n) {
        if (degree[i] == 1 and i != ind) {
            tre.push(i);
        }
    }
    vector<int> ans;
