...
void run(int t) {
    ...
    sort(all(b));
    b.erase(unique(all(b)),b.end()); // remove all duplicate elemnt 
    int ans=0;
    fora2(exclude,1,50){       // max possible answer is 50 
        vector<int> temp;
        fora(i,0,n){
            bool found=false;
            fora(j,0,a[i].sz){
                if(a[i][j]==exclude){ // we are try to make a valid set using exclude element 
                    found=true;
                    break;
                }
            }
            if(!found){
                fora(j,0,a[i].sz){
                    temp.pb(a[i][j]);  // new valid set
                }
            }
        }
        sort(all(temp));
        temp.erase(unique(all(temp)),temp.end()); // remove all duplicate elemnt
        if(temp!=b){
            ans=max(ans,(int)temp.sz);
        }
    }
    cout<<ans<<nl;
}
