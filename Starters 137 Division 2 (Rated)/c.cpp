...
void Broken_Heart(int r) {
    ...
    x = min(x, y);
    result = max(0LL, n - (lcm - x));
    int limit = min(n, lcm - x);
    int multiples = limit / x;

    result += (multiples * (multiples + 1)) / 2;
    if (n < lcm - x) {
        int boundary = x * multiples + multiples;
        if (boundary < n)
            result += (boundary - (x * multiples));
        else
            result += (n - (x * multiples));
    }
    cout << result << endl;
}
int32_t main() {
